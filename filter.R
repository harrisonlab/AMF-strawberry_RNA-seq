...
library(dplyr)
test2 <- test
dump_set <- subset(test2,V6>V5&V7>=100) # get rid of any subsequences (97% sequence identity over 100 length)
...
apply(a1,1, function(x) 
  {y=(exists(x[1],envir=e1)*1);
   y=y+(exists(x[2],envir=e1)*2);
   y=y+(exists(x[1],envir=e2)*4);
   y=y+(exists(x[2],envir=e2)*8);
	if(y==0) {
		e1[[x[1]]]="";
		e2[[x[2]]]="";
		#output.="$c[0]\n";	
